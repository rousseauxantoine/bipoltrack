...
// Menstrual cycle segmentation constants (v0.3 — fixed, see issue #60)
export const GAP_MIN    = 2;  // min days without bleeding for a new period to start
export const MIN_CYCLE  = 21; // min days between two cycle starts (anti-spotting guard)
export const PERIOD_MAX = 10; // max bleeding days before flagging as atypical
...
// Returns the number of whole days from ISO date a to ISO date b (positive when b > a).
...
/**
 * Tri-zone mood classifier — used for menstrual cycle correlation analysis (issue #60).
 * Returns 'bas' | 'stable' | 'haut' | null.
 * Three zones are all counted; transitions between any two distinct zones are mood switches.
 * Replaces the former binary 'down'/'up'/null classifier.
 */
export function moodPhaseArgile(mood) {
  if (mood == null) return null;
  if (mood < 40)   return 'bas';
  if (mood < 60)   return 'stable';
  return 'haut';
}

/**
 * Internal bi-zone helper used by the legacy projection engine (computeAvgPhaseDurationArgile /
 * computePhaseProjectionsArgile). Those functions alternate between two states ('up'/'down') and
 * are inconsistent with the tri-zone model — to be refactored in P2 (issue #60 §9).
 * @private
 */
function _moodBizone(mood) {
  if (mood == null) return null;
  if (mood < 40)   return 'down';
  if (mood >= 60)  return 'up';
  return null; // stable zone not counted by the binary projection engine
}
...
export function computeAvgPhaseDurationArgile(entries, windowDays = 90) {
  ...
  const relevant = [...entries]
    .filter(e => e.date >= cutoffStr && _moodBizone(e.mood) !== null)
  ...
  let segPhase = _moodBizone(relevant[0].mood);

  for (let i = 1; i < relevant.length; i++) {
    const p = _moodBizone(relevant[i].mood);
    if (p !== segPhase) {
...
export function computePhaseProjectionsArgile(entries, algo = '21j') {
  ...
  const sorted = [...entries]
    .filter(e => _moodBizone(e.mood) !== null)
    .sort((a, b) => a.date.localeCompare(b.date));
  ...
  for (let i = 0; i < sorted.length; i++) {
    const phase       = _moodBizone(sorted[i].mood);
  ...
  let currentPhase   = _moodBizone(sorted[0].mood);
  for (let i = 1; i < sorted.length; i++) {
    const p = _moodBizone(sorted[i].mood);
...
// ── Cycle segmentation engine (issue #60 — replaces modulo-based phasing) ────────────────

export const CYCLE_PHASES = [
  { id: 'regles',       label: 'Règles',       color: '#C0473F' },
  { id: 'folliculaire', label: 'Folliculaire', color: '#4F8A6B' },
  { id: 'ovul',         label: 'Ovulation',    color: '#D4A23A' },
  { id: 'lutéale',      label: 'Lutéale',      color: '#6F77B3' },
];
...
/**
 * Segments all entries into menstrual cycles derived from raw 'regles' and 'ovulation' tags.
 * Replaces the former modulo-based engine (computeCycleDay / phaseOf).
 *
 * Algorithm (§8, issue #60):
 *   1. Collect days bearing a 'regles' tag and sort chronologically.
 *   2. Fuse runs separated by ≤ (GAP_MIN − 1) = 1 day into a single bleeding period.
 *   3. The first day of each run is a candidate cycle start.
 *      - If < GAP_MIN days have passed since the last bleeding → candidate is rejected
 *        and absorbed into the ongoing run (fusion step handles this).
 *      - If the candidate start is < MIN_CYCLE days after the last accepted cycle start
 *        → mark as intermediate bleeding ('ambigu'); do NOT open a new cycle.
 *   4. If a bleeding run lasts > PERIOD_MAX days, flag it as 'atypical'.
 *   5. For each closed cycle, find 'ovulation' tags strictly inside ]start, end[:
 *      - 0  → status 'indetermine'
 *      - 1  → status 'phase', ovulation date recorded
 *      - >1 → status 'ambigu'
 *   6. The last cycle has end = null, status = 'en_cours'.
 *
 * Returns an array of cycle objects ordered chronologically:
 * {
 *   start:               ISO date — first day of the period (= cycle start)
 *   end:                 ISO date | null — first day of next period (null = open cycle)
 *   status:              'phase' | 'indetermine' | 'ambigu' | 'en_cours'
 *   ovulation:           ISO date | null
 *   length:              number | null — total cycle length in days
 *   follicularLength:    number | null — days from start to ovulation (only for 'phase')
 *   atypical:            boolean — bleeding duration exceeded PERIOD_MAX
 *   ambiguousBleedings:  { start, end }[] — intermediate bleedings within this cycle
 * }
 */
export function segmentCycles(entries) {
  const FUSION_GAP = GAP_MIN - 1; // = 1: gaps of exactly 1 day are fused into the same run

  // 1. Collect and sort 'regles' days
  const reglesDays = entries
    .filter(e => Array.isArray(e.menstruation) && e.menstruation.includes('regles'))
    .map(e => e.date)
    .sort();

  if (!reglesDays.length) return [];

  // 2. Fuse bleeding runs
  const runs = []; // { start: ISO, end: ISO }
  let runStart = reglesDays[0];
  let runEnd   = reglesDays[0];

  for (let i = 1; i < reglesDays.length; i++) {
    const gap = dayDiff(runEnd, reglesDays[i]);
    if (gap <= FUSION_GAP) {
      runEnd = reglesDays[i]; // extend current run
    } else {
      runs.push({ start: runStart, end: runEnd });
      runStart = reglesDays[i];
      runEnd   = reglesDays[i];
    }
  }
  runs.push({ start: runStart, end: runEnd });

  // 3. Build cycles from runs
  const cycles = [];
  let lastCycleStart = null;

  for (const run of runs) {
    const bleedingDuration = dayDiff(run.start, run.end) + 1;
    const atypical = bleedingDuration > PERIOD_MAX;

    if (lastCycleStart === null) {
      // Very first period: open first cycle
      cycles.push({
        start: run.start, end: null, status: 'en_cours',
        ovulation: null, length: null, follicularLength: null,
        atypical, ambiguousBleedings: [],
      });
      lastCycleStart = run.start;
    } else {
      const distFromLastStart = dayDiff(lastCycleStart, run.start);
      if (distFromLastStart < MIN_CYCLE) {
        // Too close to the last cycle start → intermediate bleeding, don't open new cycle
        cycles[cycles.length - 1].ambiguousBleedings.push({ start: run.start, end: run.end });
      } else {
        // Valid new cycle start: close the previous cycle, open a new one
        const prev = cycles[cycles.length - 1];
        prev.end    = run.start;
        prev.length = dayDiff(lastCycleStart, run.start);
        // status will be set below once ovulations are processed
        cycles.push({
          start: run.start, end: null, status: 'en_cours',
          ovulation: null, length: null, follicularLength: null,
          atypical, ambiguousBleedings: [],
  ...
  // 4. Assign ovulation and finalize status for each cycle
  const ovulDays = entries
    .filter(e => Array.isArray(e.menstruation) && e.menstruation.includes('ovulation'))
    .map(e => e.date)
    .sort();

  for (const cycle of cycles) {
    if (cycle.end !== null) {
      // Closed cycle: ovulations must lie strictly inside ]start, end[
      const ovuls = ovulDays.filter(d => d > cycle.start && d < cycle.end);
      if (ovuls.length === 0) {
        cycle.status = 'indetermine';
      } else if (ovuls.length === 1) {
        cycle.status          = 'phase';
        cycle.ovulation       = ovuls[0];
        cycle.follicularLength = dayDiff(cycle.start, ovuls[0]);
      } else {
        cycle.status = 'ambigu'; // multiple ovulations or incoherent placement
      }
    }
    // Open (en_cours) cycle: record ovulation if exactly one is present after the start
    if (cycle.end === null) {
      const ovuls = ovulDays.filter(d => d > cycle.start);
      if (ovuls.length === 1) {
        cycle.ovulation = ovuls[0];
      }
      // status stays 'en_cours' regardless
    }
  }
...
/**
 * Computes the φ (phi) coordinate for a date within a phaseable cycle.
 *
 *   Follicular phase: φ = (j − Mi) / (Oi − Mi)  ∈ [0, 1[
 *   Ovulation day:    φ = 1 exactly
 *   Luteal phase:     φ = 1 + (j − Oi) / (Mi+1 − Oi)  ∈ (1, 2[
 *
 * Returns null when:
 *   - cycle status is not 'phase'
 *   - date is outside the cycle's bounds
 *   - the cycle is open (en_cours) and the date is in the luteal phase (end unknown)
 */
export function computeCyclePhi(dateStr, cycle) {
  if (!cycle?.ovulation) return null;
  if (cycle.status !== 'phase') return null;
  if (dateStr < cycle.start) return null;
  if (cycle.end && dateStr >= cycle.end) return null;

  const O = cycle.ovulation;

  if (dateStr < O) {
    // Follicular phase
    const follLength = dayDiff(cycle.start, O);
    if (follLength === 0) return 0;
    return dayDiff(cycle.start, dateStr) / follLength;
  }
  if (dateStr === O) return 1;

  // Luteal phase
  if (!cycle.end) return null; // can't compute without the cycle end
  const lutLength = dayDiff(O, cycle.end);
  if (lutLength === 0) return 1;
  return 1 + dayDiff(O, dateStr) / lutLength;
}

/**
 * Builds a DayCell[] for every day in [startDate, endDate] (ISO strings, inclusive).
 * Phases are derived from raw 'regles'/'ovulation' tags via segmentCycles — not from
 * cycleSettings averages. Replaces the former modulo-based buildMoodCycleCells (issue #60 P0).
 *
 * Each cell:
 *   date        — ISO date string
 *   moodLevel   — 1 | 2 | 3 | null
 *   menstruation — string[]
 *   cycleStatus  — 'phase'|'indetermine'|'ambigu'|'en_cours'|null  (null = before first cycle)
 *   phase        — 'folliculaire'|'lutéale'|null
 *   phi          — number ∈ [0, 2[ | null
 *   isPredicted  — true for days in the open (current) cycle
 *   confidence   — 'high' (phaseable cycle) | 'low' (open/indeterminate) | 'none' (no cycle)
 *
 * @deprecated cycleDay is no longer computed and is always null.
 */
export function buildMoodCycleCells(entries, startDate, endDate) {
  ...
  for (let ms = startMs; ms <= endMs; ms += MS_PER_DAY) {
    ...
    // Find the cycle that owns this date
    const cycle = cycles.find(c => date >= c.start && (c.end === null ? true : date < c.end)) ?? null;

    const moodLevel    = entry ? moodLevelFrom(entry) : null;
    const menstruation = entry?.menstruation ?? [];
    const cycleStatus  = cycle ? cycle.status : null;
    const isPredicted  = cycle ? cycle.end === null : false;
    ...
    if (cycle) {
      switch (cycle.status) {
        case 'phase':
          confidence = 'high';
          phase      = date < cycle.ovulation ? 'folliculaire' : 'lutéale';
          phi        = computeCyclePhi(date, cycle);
          break;
        case 'en_cours':
          confidence = 'low';
          if (cycle.ovulation) {
            phase = date < cycle.ovulation ? 'folliculaire' : 'lutéale';
            phi   = null; // luteal φ requires cycle end; follicular φ also kept null for provisional
          } else {
            phase = 'folliculaire'; // folliculaire présumée (P0-3)
          }
          break;
        case 'indetermine':
          confidence = 'low';
          // phase stays null — indéterminé affiché explicitement (P0-3)
          break;
        case 'ambigu':
        default:
          // confidence stays 'none' — ambigu displayed explicitly, no phase inferred
          break;
      }
    }
    ...
    cells.push({
      ...
      phase,
      phi,
      cycleDay:    null, // @deprecated — no longer computed from modulo
      isPredicted,
      confidence,
    });
  }
...
// ── Deprecated cycle helpers (modulo-based, replaced by segmentCycles) ──────────────

/**
 * @deprecated Use segmentCycles() instead (issue #60 P0).
 * Returns the 1-based day number within the menstrual cycle using a fixed-length modulo approach.
 */
...
/**
 * @deprecated Use segmentCycles() instead (issue #60 P0).
 * Returns the phase id for the given cycle day using a fixed ovulation estimate (L − 14).
 */
...
/**
 * @deprecated Use per-cycle confidence from segmentCycles() instead (issue #60 P1-5).
 * Returns confidence level ('high'|'low'|'none') based on cycleSettings history reliability.
 */
