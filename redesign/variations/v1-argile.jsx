...
// ───── Journal · phase 3 — Mot ─────
function ArgileJournalMot({ onClose }) {
...
// Nouvelle échelle : Tristesse · Sérénité · Euphorie
const CARNET_HUMEUR_ZONES = [
  { id: 'tristesse', label: 'Tristesse', color: '#9B7A7A', moodMax: 35 },
  { id: 'sérénité',  label: 'Sérénité',  color: '#AE9F8C', moodMax: 65 },
  { id: 'euphorie',  label: 'Euphorie',  color: '#B85839', moodMax: 100 },
];
function carnetHumeurOf(v) {
  return CARNET_HUMEUR_ZONES.find(z => v <= z.moodMax) || CARNET_HUMEUR_ZONES[2];
}

function ArgileStats() {
  // Données mock — courbe sur 30 jours (oscillation réaliste)
  const days = Array.from({ length: 30 }, (_, i) => {
    const r = Math.sin(i * 0.4) * 30 + Math.cos(i * 0.27) * 15 + 50 + (i > 22 ? 8 : 0);
    return Math.max(5, Math.min(95, Math.round(r)));
  });

  // Médiane de la série mock → Sérénité (la plupart des valeurs sont autour de 50)
  const sorted = [...days].sort((a, b) => a - b);
  const medianVal = sorted[Math.floor(sorted.length / 2)];
  const medianZone = carnetHumeurOf(medianVal);

  // Distribution par zone (nouvelle échelle)
  const zoneMock = [
    { zone: 'Tristesse', days: 9,  color: '#9B7A7A' },
    { zone: 'Sérénité',  days: 14, color: '#AE9F8C' },
    { zone: 'Euphorie',  days: 7,  color: '#B85839' },
  ];
  ...
  return (
    <div style={{ padding: '20px 24px 0' }}>
      ...
      <p style={{ fontSize: 13, color: ARGILE.muted, margin: '0 0 24px' }}>30 entrées ces 30 derniers jours.</p>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontFamily: 'Instrument Serif, serif', fontSize: 32, color: medianZone.color, lineHeight: 1, marginTop: 4, fontStyle: 'italic' }}>{medianZone.label}</div>
          </div>
        ...
        <svg viewBox="0 0 300 90" style={{ width: '100%', height: 90 }}>
          ...
          <polyline
            ...
            fill="none" stroke={ARGILE.muted} strokeWidth="1.5" opacity="0.25"
          />
          {days.map((d, i) => {
            const x = (i / (days.length - 1)) * 300;
            const y = 90 - (d / 100) * 80 - 5;
            const zone = carnetHumeurOf(d);
            return <circle key={i} cx={x} cy={y} r={i === days.length - 1 ? 4.5 : 2.8} fill={zone.color} opacity={0.55 + (i / days.length) * 0.45} />;
          })}
        </svg>
      ...
      {/* zones du mois — nouvelle échelle */}
...
function ArgileApp({ initialScreen = 'journal', tweaks = {} }) {
  ...
  else if (screen === 'mot')     body = <ArgileJournalMot onClose={() => setScreen('done')} />;
